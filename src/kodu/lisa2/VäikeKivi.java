package kodu.lisa2;

public class VäikeKivi implements Ruut{

    int morad = 0;
    private boolean ruudulTegelane = false;
    final static int katki = 3;
    @Override
    public boolean võtaVastu(Tegelane tegelane) {
        if(!tegelane.omabEset("kirka")) return false;
        ruudulTegelane = true;
        if(morad < katki) {
            morad = morad + 1;
            return false;
        } else {
            morad = morad+1;
            return true;
        }
    }

    @Override
    public boolean tegelaneVõibLahkuda() {
        if(morad >= katki) {
            ruudulTegelane = true;
            return true;
        }
        return false;
    }

    @Override
    public void saadaTegelaneMinema() {
        if(!ruudulTegelane) throw new RuntimeException("Ruudul pole tegelast");
        else ruudulTegelane=false;
    }

    @Override
    public void joonista(int x, int y) {
        if(morad == 0) {
            System.out.println("väike_kivi.png " +  "(" + x + ", " + y + ")");
        } else if (morad < katki) {
            System.out.println("väike_kivi_" + morad + "_mõra.png " +  "(" + x + ", " + y + ")");
        } else {
            System.out.println("väike_kivi_puru.png " +  "(" + x + ", " + y + ")");
        }
    }
}
